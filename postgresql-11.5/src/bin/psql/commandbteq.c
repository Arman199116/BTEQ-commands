...
static dotResult exec_command_a(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_C(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_connect(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_cd(PsqlScanState scan_state, bool active_branch,
				const char *cmd);
static dotResult exec_command_conninfo(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_copy(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_copyright(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_crosstabview(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_d(PsqlScanState scan_state, bool active_branch,
			   const char *cmd);
static dotResult exec_command_edit(PsqlScanState scan_state, bool active_branch,
				  PQExpBuffer query_buf, PQExpBuffer previous_buf);
static dotResult exec_command_ef_ev(PsqlScanState scan_state, bool active_branch,
				   PQExpBuffer query_buf, bool is_func);
static dotResult exec_command_echo(PsqlScanState scan_state, bool active_branch,
				  const char *cmd);
static dotResult exec_command_elif(PsqlScanState scan_state, ConditionalStack cstack,
				  PQExpBuffer query_buf);
static dotResult exec_command_else(PsqlScanState scan_state, ConditionalStack cstack,
				  PQExpBuffer query_buf);
static dotResult exec_command_endif(PsqlScanState scan_state, ConditionalStack cstack,
				   PQExpBuffer query_buf);
static dotResult exec_command_encoding(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_errverbose(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_f(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_g(PsqlScanState scan_state, bool active_branch,
			   const char *cmd);
static dotResult exec_command_gdesc(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_gexec(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_gset(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_help(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_html(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_include(PsqlScanState scan_state, bool active_branch,
					 const char *cmd);
static dotResult exec_command_if(PsqlScanState scan_state, ConditionalStack cstack,
				PQExpBuffer query_buf);
static dotResult exec_command_list(PsqlScanState scan_state, bool active_branch,
				  const char *cmd);
static dotResult exec_command_lo(PsqlScanState scan_state, bool active_branch,
				const char *cmd);
static dotResult exec_command_out(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_print(PsqlScanState scan_state, bool active_branch,
				   PQExpBuffer query_buf, PQExpBuffer previous_buf);
static dotResult exec_command_password(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_prompt(PsqlScanState scan_state, bool active_branch,
					const char *cmd);
static dotResult exec_command_pset(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_quit(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_reset(PsqlScanState scan_state, bool active_branch,
				   PQExpBuffer query_buf);
static dotResult exec_command_s(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_set(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_setenv(PsqlScanState scan_state, bool active_branch,
					const char *cmd);
static dotResult exec_command_sf_sv(PsqlScanState scan_state, bool active_branch,
				   const char *cmd, bool is_func);
static dotResult exec_command_t(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_T(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_timing(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_unset(PsqlScanState scan_state, bool active_branch,
				   const char *cmd);
static dotResult exec_command_write(PsqlScanState scan_state, bool active_branch,
				   const char *cmd,
				   PQExpBuffer query_buf, PQExpBuffer previous_buf);
static dotResult exec_command_watch(PsqlScanState scan_state, bool active_branch,
				   PQExpBuffer query_buf, PQExpBuffer previous_buf);
static dotResult exec_command_x(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_z(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_shell_escape(PsqlScanState scan_state, bool active_branch);
static dotResult exec_command_slash_command_help(PsqlScanState scan_state, bool active_branch);
...
exec_command_C(PsqlScanState scan_state, bool active_branch)
{
	...
	if (active_branch)
	{
		...
		success = do_psetbteq("title", opt, &pset.popt, pset.quiet);
		free(opt);
	}
	else
...
exec_command_f(PsqlScanState scan_state, bool active_branch)
{
	...
	if (active_branch)
	{
		...
		success = do_psetbteq("fieldsep", fname, &pset.popt, pset.quiet);
		free(fname);
	}
	else
		ignore_slash_options(scan_state);
...
exec_command_html(PsqlScanState scan_state, bool active_branch)
{
	...
	if (active_branch)
	{
		if (pset.popt.topt.format != PRINT_HTML)
			success = do_psetbteq("format", "html", &pset.popt, pset.quiet);
		else
			success = do_psetbteq("format", "aligned", &pset.popt, pset.quiet);
	}
...
exec_command_include(PsqlScanState scan_state, bool active_branch, const char *cmd)
{
	...
	if (active_branch)
	{
		...
		else
		{
			...
			expand_tilde(&fname);
			success = (process_filebteq(fname, include_relative) == EXIT_SUCCESS);
			free(fname);
		}
	}
	else
...
exec_command_pset(PsqlScanState scan_state, bool active_branch)
{
	...
	if (active_branch)
	{
		...
		if (!opt0)
		{
			...
			success = do_psetbteq(opt0, opt1, &pset.popt, pset.quiet);
...
exec_command_t(PsqlScanState scan_state, bool active_branch)
{
	...
	if (active_branch)
	{
		...
		success = do_psetbteq("tuples_only", opt, &pset.popt, pset.quiet);
		free(opt);
	}
	else
		ignore_slash_options(scan_state);
...
exec_command_T(PsqlScanState scan_state, bool active_branch)
{
	...
	if (active_branch)
	{
		...
		success = do_psetbteq("tableattr", value, &pset.popt, pset.quiet);
		free(value);
	}
	else
		ignore_slash_options(scan_state);
...
exec_command_x(PsqlScanState scan_state, bool active_branch)
{
	...
	if (active_branch)
	{
		...
		success = do_psetbteq("expanded", opt, &pset.popt, pset.quiet);
		free(opt);
	}
	else
		ignore_slash_options(scan_state);
 ...
 */
int
process_filebteq(char *filename, bool use_relative_path)
{
	FILE	   *fd;
	int			result;
	char	   *oldfilename;
 ...
 */
bool
do_psetbteq(const char *param, const char *value, printQueryOpt *popt, bool quiet)
{
	size_t		vallen = 0;
