 ...
 *
 * This file declares the BteqScanStateData structure used by bteqscan.l
 * and shared by other lexers compatible with it, such as bteqscandot.l.
 *
 ...
 * bteqscan_emit() subroutine is responsible for looking back to the original
 ...
 * relevant state is in the yyscanner_t attached to the BteqScanState;
 ...
 *-------------------------------------------------------------------------
 */
#ifndef BTEQSCAN_INT_H
#define BTEQSCAN_INT_H
 ...
 */
#ifndef YY_BTEQ_TYPEDEF_YY_BUFFER_STATE
#define YY_BTEQ_TYPEDEF_YY_BUFFER_STATE
typedef struct yy_buffer_state *YY_BUFFER_STATE;
#endif
#ifndef YY_BTEQ_TYPEDEF_YY_SCANNER_T
#define YY_BTEQ_TYPEDEF_YY_SCANNER_T
...
typedef struct StackElemBteq
{
    ...
    struct StackElemBteq *next;
} StackElemBteq;

/*
 * All working state of the lexer must be stored in BteqScanStateData
 * between calls.  This allows us to have multiple open lexer operations,
 * which is needed for nested include files.  The lexer itself is not
 * recursive, but it must be re-entrant.
 */
typedef struct BteqScanStateData
{
    yyscan_t    scanner;        /* Flex's state for this BteqScanState */

    PQExpBuffer output_buf;        /* current output buffer */

    StackElemBteq  *buffer_stack;    /* stack of variable expansion buffers */
     ...
     * reset by bteq_scan_reset.  start_state is adopted by yylex() on entry,
     ...
     */
    const BteqScanCallbacks *callbacks;
    void       *cb_passthrough;
} BteqScanStateData;
...
extern void bteqscan_pop_buffer_stack(BteqScanState state);
extern void bteqscan_select_top_buffer(BteqScanState state);
extern bool bteqscan_var_is_current_source(BteqScanState state,
                               const char *varname);
extern YY_BUFFER_STATE bteqscan_prepare_buffer(BteqScanState state,
                        const char *txt, int len,
                        char **txtcopy);
extern void bteqscan_emit(BteqScanState state, const char *txt, int len);
extern char *bteqscan_extract_substring(BteqScanState state,
                           const char *txt, int len);
extern void bteqscan_escape_variable(BteqScanState state,
                         const char *txt, int len,
                         BteqScanQuoteType quote);
extern void bteqscan_test_variable(BteqScanState state,
                       const char *txt, int len);

#endif                            /* BTEQSCAN_INT_H */
